...
/**
 * Generic type for any DynamoDB query spec that has a query field
 */
type DynamoDBQuerySpec = {
  query?: string;
};

/**
 * A hook for managing the `query` state in DynamoDB query specs. Returns the `query` value, along with
 ...
 */
export function useQueryState<T extends DynamoDBQuerySpec>(
